   ...
   """
    ...
    def main(self,optuna_params={}):
        ...
        if self.args.optuna:
            ...
            self.args.previous_model_path = optuna_params['previous_model_path']
        ...
        with open(os.path.join(self.args.previous_model_path, "model/model_params.json"), 'r') as file:
            previous_args= json.load(file)
        self.args.last_kernel_num =previous_args["args"]["last_kernel_num"]
        self.args.drop_out =previous_args["args"]["drop_out"]
        params = dict(
        ...
        model_torch = os.path.join(self.args.previous_model_path, "model/model.torch")
        self.model.load_state_dict(torch.load(model_torch))
...
def train_optuna(trial):
    ...
    """    
    previous_model_path = []
    path=os.path.join(os.path.dirname(os.getcwd()), 'code/maml_save')
    save_model_path = os.listdir(path)
    for i in save_model_path:
            previous_model_path.append(os.path.join(path,i))       
    params_train = {
        ...
        "previous_model_path" : trial.suggest_categorical("previous_model_path",previous_model_path),
    }

    parser = argparse.ArgumentParser(description='Automatic parameter setting')
    parser.add_argument('--config_path', type=str, default=os.path.join(os.path.dirname(os.getcwd()), 'code/ft_param.json'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str,
                        default=os.path.join(os.path.dirname(os.getcwd()), 'code/ft_param.json'))
    args = parser.parse_args()
